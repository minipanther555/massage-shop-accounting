...
function populateDropdowns() {
  try {
    ...
    const nextInLineName = nextInLine ? nextInLine.name : null;
    ...
    console.log('🔍 DROPDOWNS POPULATED - Next in line:', nextInLineName);
  ...
  function updateDurationOptions() {
    ...
    if (location && serviceName) {
      ...
      const serviceOptions = CONFIG.settings.services.filter((s) => s.name === serviceName && s.location === location);
  ...
  function updatePricing() {
    ...
    if (location && serviceName && duration) {
      const service = CONFIG.settings.services.find((s) => s.name === serviceName && s.duration == duration && s.location === location);
