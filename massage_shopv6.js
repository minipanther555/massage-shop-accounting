...
const CONFIG = {
  ...
  function onEdit(e) {
    ...
    console.log("onEdit triggered:", range.getA1Notation(), "value:", e.value);
    
    // Handle checkbox clicks
    if (e.value === "TRUE") {
      const cell = range.getA1Notation();
      console.log("Checkbox clicked:", cell);
    ...
    if (range.getA1Notation() === "F6" && e.value) {
      console.log("Payment method changed, populating time dropdowns");
...
function resetDailyEntrySheet() {
  ...
  // Clear recent transactions log DATA ONLY (not the header) - exactly 4 rows of data (A28:I31)
...
function loadTransactionForCorrection() {
  ...
  function submitTransaction() {
    ...
    // Ensure customerContact is a string and handle any special characters
    const safeCustomerContact = customerContact ? String(customerContact).trim() : "";
...
function setupDailyEntrySheet(ss) {
  ...
  // SECTION 5: RECENT TRANSACTIONS LOG (A27:I31) - EXACTLY 5 ROWS TOTAL
  applyHeaderStyle(sheet.getRange("A27:I27").merge().setValue("RECENT TRANSACTIONS LOG"));
  
  // Fixed QUERY formula - more robust to handle multiple transactions
  // Shows: Payment Method, Masseuse, Service, Amount, Fee, Start Time, End Time
  // Limited to exactly 4 rows of data (5 total including header)
  const recentLogFormula = `=IFERROR(QUERY('${CONFIG.sheets.masterLog}'!A:L, "SELECT G, D, E, F, H, I, J WHERE L = 'ACTIVE' ORDER BY B DESC LIMIT 4 LABEL G 'Payment', D 'Masseuse', E 'Service', F 'Amount', H 'Fee', I 'Start', J 'End'", 1), "No transactions yet.")`;
  ...
  sheet.getRange("F28:I31").setNumberFormat("h:mm am/pm");
