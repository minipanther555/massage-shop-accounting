...
*   **Service, Duration, and Payment Button Layer (`renderServiceButtons()`, `openVariantPanel()`, `sortVariantServices()`, `renderDurationButtons()`, `renderPaymentButtons()`, `selectServiceValue()`, `selectDurationValue()`, `selectPaymentValue()`, `sortComboServices()`):**
...
*   **Logic:** `renderServiceButtons()` reads the current hidden `service` options and groups actual menu names into available category buttons via `getServiceCategory()` — a positional first-match against `serviceCategoryDefinitions`. Order in that array is load-bearing: `coconut` precedes `oil` so `Coconut lovers - coconut oil massage` (name contains both substrings) lands in Coconut, not Oil. Tapping a category button opens the shared sub-panel (`#variant-service-panel` / `#variant-service-buttons` with dynamic `#variant-service-title`) when the category resolves to two or more services in the current catalog; single-service categories select on one tap without opening the sub-panel. `openVariantPanel()` renders one button per service in that category — sorted by `sortVariantServices()` (regular first, `Deep …` variants last, alphabetical tie-break) for non-combo categories, or by `sortComboServices()` (`Foot + back, neck & shoulder` first) for the Combo category. This means the generic Oil category opens `Oil massage` + `Deep oil` sub-buttons instead of auto-selecting one, and Aroma opens `Aroma massage` + `Deep aroma`. `selectServiceValue()` still writes the exact old English service value into `#service` and dispatches the same `change` event used by the legacy cascade. Combo sub-buttons render the Thai hint as the large primary line and the canonical English service name as smaller secondary text; non-combo variant sub-buttons render the full display name so both variants of Oil / Aroma read clearly. `renderDurationButtons()` then reads the valid hidden `duration` options for that service, and `selectDurationValue()` writes the exact duration value into `#duration` before pricing/time updates run. `renderPaymentButtons()` reads the hidden `#payment` options, renders them as a separate full-width row under service/duration selection, and `selectPaymentValue()` writes the exact payment method into `#payment` before dispatching the legacy change event.
...
### Money mode — `tippableTransactions()`, `renderMoneyPanel()`, `setMoneyKind()`, `updateMoneySubmitState()`, `submitMoneyEntry()` (RIT-UI-003, 2026-08-19)

- **Purpose:** Let reception record the two kinds of money that are not massage revenue — a **tip** (`TIP`) the customer paid by card or app, which the shop hands to the masseuse in cash, and a **miscellaneous charge** (`MISC_INCOME`) such as tiger balm at ฿50.
- **Returns / Renders:** `#money-mode-panel`, a fourth reception mode alongside walk-in, booking and extend. Populates `#money-parent` and `#money-payment`; toggles `#money-kind-tip` / `#money-kind-misc`; enables or disables `#money-submit-button`; on success calls `loadTodayData()` then `updateAllDisplays()`.
- **Usage & Logic Notes:**
  - **One client path.** Both kinds POST through `api.createAddOn()` to `POST /api/transactions/add-ons`, the same client method the two service kinds already use. A second path would drift from the server contract the moment either side changed.
  - **The day figure refreshes through the page's existing mechanism, not a new one.** `updateAllDisplays()` → `updateQuickSummary()` is what already rewrites `#today-revenue`; money mode calls it and adds no second refresh. The browser spec counts `updateQuickSummary()` calls rather than reading the figure alone, because a figure that happens to match reads green over an update that never ran (RIT-UI-001's Discovery).
  - **The submit button is disabled until the entry can actually be saved.** The server requires a positive `amount` for both kinds, a `parent_transaction_id` for `TIP` (RIT-MONEY-001), a non-blank `description` for `MISC_INCOME` (RIT-MONEY-003), and a `payment_method` whenever `payment_status` is `PAID`. Each omission is answered with a `400` whose text is not reception's language, so `updateMoneySubmitState()` gates on all four and `submitMoneyEntry()` re-checks them behind the button. The description is `.trim()`ed on both paths, so whitespace cannot satisfy the check — the same trim the server applies.
  - **`tippableTransactions()` accepts `ACTIVE` **and** `CORRECTED`, deliberately.** The extend panel's `extendableTransactions()` filters `status === 'ACTIVE'` only, which is right for a service extension. A tip is looser: `FR-005`'s edge case says a tip on a transaction later edited follows the parent's correction chain, whose live row is `CORRECTED`, and the server's `isLiveWork()` accepts exactly that pair. Offering only `ACTIVE` rows here would hide transactions the ledger would have accepted. **The two candidate lists must not be merged.**
  - **A tip must name its transaction; a charge need not.** `#money-parent`'s first option is `ไม่ผูกกับรายการใด` (not linked to any entry) with an empty value — legitimate for `MISC_INCOME` (`FR-006` processing logic 2) and refused for `TIP` by both the gate and the server.
  - **Money mode does NOT hide the intake form, and extend mode does.** `#transaction-mode-switch` and both panels live **inside** `#transaction-form`, so `intakeForm.hidden = true` takes the panel and the mode buttons with it. Extend mode has that defect today (its panel computes `display: block` under a `display: none` ancestor). Money mode therefore leaves the form visible and renders its panel above it. See the Discovery in the steps ledger.
  - **Nothing on this path touches the roster, the queue or a masseuse's fee.** `AC-009` is proven by re-fetching `/api/staff/current-status` after each entry and asserting the next-in-queue label is byte-identical to what it was before.
  - Every interpolated value in `#money-parent`'s options — transaction id, masseuse name, service type — passes through `escapeBookingText()`, and amounts through `Number(...).toFixed(2)`. Hints, labels and `#money-error` write through `textContent`.
  - **`populatePaymentMethodOptions(selectId)` is shared** by `populateExtendPaymentOptions()` and `populateMoneyPaymentOptions()`; the two were byte-identical apart from the element id.
...
**Resolution (2026-07-22):** Added `getPreferredCategoryService()` to select exact active `Oil massage` for the generic Oil category while preserving the first available fallback if that catalog row is unavailable. The backend catalog and promotion data remain unchanged.

**Superseded (2026-08-09, ISVSB-UI-001):** `getPreferredCategoryService()` and its `preferredServiceNames` map were removed. The generic Oil category now opens a sub-panel with `Oil massage` and `Deep oil` sub-buttons instead of auto-selecting — reception picks the specific variant, and each sub-button routes through the same `selectServiceValue()` path so the server quote still receives the exact service name and reaches the correct promotion rows. To keep `Coconut lovers - coconut oil massage` out of the Oil sub-panel, `serviceCategoryDefinitions` was reordered so `coconut` precedes `oil` (positional first-match). This also surfaces the Coconut category button, which had been silently hidden because its only service was previously being stolen by Oil. See `00-project-docs/feature-specifications/intake-service-variant-sub-buttons.md`.
...
### RIT-UI-003: Reception Could Not Record a Tip or an Extra Charge (2026-08-19)
**Bug Summary:** The ledger accepted `TIP` and `MISC_INCOME` from `RIT-MONEY-001` onward, but the New Customer page offered no way to enter either. A customer tipping on a card, or buying tiger balm at ฿50 on top of a massage, could not be recorded at all.

**Validated Hypothesis:** The missing piece was a fourth reception mode, not a change to any existing one. Both kinds already have a server contract, a client method (`api.createAddOn`) and a refresh path (`updateAllDisplays()`); only the control was absent.

**Invalidated Hypotheses:**
- *A tip can reuse the extend panel's candidate list.* It cannot — `extendableTransactions()` filters `status === 'ACTIVE'`, and a tip is legitimately allowed on a `CORRECTED` parent (`FR-005` edge case). `tippableTransactions()` accepts both.
- *Money mode should hide the intake form, as extend mode does.* Hiding `#transaction-form` hides the mode-switch buttons and both panels with it, because all of them are inside that form. Extend mode has this defect today; money mode does not copy it.
- *Asserting the day figure changed is enough.* It is not — the figure can match the expected value over an update that never ran. The spec patches `updateQuickSummary()` with a counting wrapper and asserts the call count as well.
- *A blank description can fall back to something.* It cannot — a `MISC_INCOME` row's description **is** its `service_type` (RIT-MONEY-003), and the server `400`s a blank one. The submit button is disabled instead.

**Resolution:** `#money-mode-button` and `#money-mode-panel` added to both intake templates, with `tippableTransactions()`, `renderMoneyPanel()`, `populateMoneyParentOptions()`, `populateMoneyPaymentOptions()`, `setMoneyKind()`, `updateMoneySubmitState()`, `showMoneyError()` and `submitMoneyEntry()`. Verified by `tests/e2e/transaction.money-entry.spec.js` (real Chromium, real server, real database: records a ฿100 card tip and a ฿50 tiger-balm charge, asserts `#today-revenue` moves 850 → 950 → 1000 with no reload, and asserts the next-in-queue label is unchanged after **both** entries) and by the mirror contract `__tests__/transaction.money-entry.present.test.js`. Spec: `FR-005`, `FR-006`, `AC-006`, `AC-008`, `AC-009`.
