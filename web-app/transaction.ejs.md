...
### Stale-Staff Marker Mirrored From transaction.html (2026-08-19, RIT-UI-001)
- **Bug Summary:** `transaction.html` moved `renderMasseuseDropdown()` out of the fetch error path in `refreshRosterForDropdown()` (`Promise.all` → `Promise.allSettled`), added `setStaffStaleMarker()`, and added the `#staff-stale-marker` element to the staff field group. The mirror had to receive the identical change or the contract test `__tests__/transaction.stale-marker.present.test.js` (`test.each([html, ejs])`) fails, alongside the existing `transaction.walkin-refresh.present.test.js`.
- **Validated Hypothesis:** One commit touching both files keeps the mirror invariant. `diff web-app/transaction.html web-app/transaction.ejs` still shows only the two CSRF-token placeholder differences (L6 `<meta>` and L50 hidden input).
- **Resolution:** Applied identically to this file. The behaviour is defined once in [`web-app/transaction.html.md`](web-app/transaction.html.md) §2 under `refreshRosterForDropdown()` / `setStaffStaleMarker()`; this doc deliberately does not re-derive it.

### Dropdown Empty State Mirrored From transaction.html (2026-08-19, RIT-UI-002)
- **Bug Summary:** `transaction.html` removed `renderMasseuseDropdown()`'s page-load name-list fallback (`orderedMasseuses.length ? orderedMasseuses : CONFIG.settings.masseuses` → `orderedMasseuses`), added `staffInformationFailedToLoad()` and `setStaffEmptyState()`, and added the `#staff-empty-state` element to the staff field group beneath `#staff-stale-marker`. The mirror had to receive the identical change or the contract test `__tests__/transaction.dropdown-empty-state.present.test.js` (`test.each([html, ejs])`) fails.
- **Validated Hypothesis:** One commit touching both files keeps the mirror invariant. `diff web-app/transaction.html web-app/transaction.ejs` still shows only the two CSRF-token placeholder differences (L6 `<meta>` and L50 hidden input).
- **Resolution:** Applied identically to this file by one script driven from one pair of anchors. The behaviour is defined once in [`web-app/transaction.html.md`](web-app/transaction.html.md) §2 under `renderMasseuseDropdown()` / `staffInformationFailedToLoad()` / `setStaffEmptyState()`; this doc deliberately does not re-derive it.

### Money Mode Mirrored From transaction.html (2026-08-19, RIT-UI-003)
- **Bug Summary:** `transaction.html` gained a fourth reception mode for money that is not massage revenue — `#money-mode-button`, `#money-mode-panel`, and the helpers `tippableTransactions()`, `renderMoneyPanel()`, `populateMoneyParentOptions()`, `populateMoneyPaymentOptions()`, `setMoneyKind()`, `updateMoneySubmitState()`, `showMoneyError()` and `submitMoneyEntry()` — plus the shared `populatePaymentMethodOptions(selectId)` that `populateExtendPaymentOptions()` now calls. The mirror had to receive the identical change or the contract test `__tests__/transaction.money-entry.present.test.js` (`test.each([html, ejs])`) fails.
- **Validated Hypothesis:** One commit touching both files keeps the mirror invariant. `diff web-app/transaction.html web-app/transaction.ejs` still shows only the two CSRF-token placeholder differences (L6 `<meta>` and L50 hidden input).
- **Resolution:** Applied identically to this file by one script driven from one pair of anchors. The behaviour is defined once in [`web-app/transaction.html.md`](web-app/transaction.html.md) §2 under "Money mode"; this doc deliberately does not re-derive it.
