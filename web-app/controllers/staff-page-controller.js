...
function createStaffPageController({ fetch, clock, logger, singleFlight = null }) {
  ...
  const updateAvailableStaffDropdown = () => {
    ...
    const requestPromise = api.getAllStaff().then(allStaffNames => {
      ...
      logger.log(`✅ Populated dropdown with ${availableStaff.length} available staff out of ${allStaffNames.length} total staff`);
    }).catch(error => {
  ...
  const updateRosterDisplay = () => {
    ...
    // This is the source of the duplicate request
