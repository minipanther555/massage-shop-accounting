...
- Current branch usernames generated by the login page:
  - `reception_top_thai_55` / `manager_top_thai_55` -> Top Thai 55
...
- Current local branch selection does not by itself create separate SQLite files. Production rollout requires database-per-branch routing so each branch has the same schema in its own database file and therefore its own staff list and operational data.
