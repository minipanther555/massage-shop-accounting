...
19. **[Feature] Service Pricing, Promotions, and Time-Window Discounts.**
    *   **Status**: 🟡 PARTIAL - branch-43 in-shop time-window promotion implemented; catalog price/commission updates and receptionist-only loyalty/return rules remain open (2026-07-21)
    ...
    *   **Validation**: Unit tests cover Bangkok boundaries and missing configuration; integration tests prove final payment/audit storage while normal commission remains earned, including a Home Service non-application regression. Live branch-43 browser/API smoke remains required after deployment.
