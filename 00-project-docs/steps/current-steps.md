...
23. **[Feature] Transaction Correction and Walk-In Cancellation.**
    *   **Status**: ✅ TCR-001 current-day correction and TCR-002 normal walk-in cancellation shipped (2026-07-23); booking-backed cancellation remains intentionally out of scope pending semantics.
    *   **Required**: Reception can correct a saved walk-in by preserving the original audit row and creating a corrected replacement, and can next cancel a saved walk-in from the same edit workflow when the customer leaves before the massage starts.
    *   **Dependencies**: `00-project-docs/feature-specifications/transaction-correction-operational-reversal.md`, `00-project-docs/steps/transaction-correction-operational-reversal-steps.md`, transaction routes, current-status staff availability, mirrored New Customer templates, Home/Daily Summary/report/Payday transaction reads, and DB-Ops only if implementation needs schema or data mutation.
