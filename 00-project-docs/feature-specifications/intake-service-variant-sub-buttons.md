...
- **AC-007.** `CONFIG.settings.services`, `/api/services`, `web-app/shared.js`, and `web-app/admin-services.html` are byte-identical to before this feature (grep-verifiable: only `web-app/transaction.html` appears in `git diff --name-only`).
