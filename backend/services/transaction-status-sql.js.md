...
*   **Consumed by:** `backend/routes/transactions.js` — four sites: the correction replacement picker's workload subquery (`:21`) and busy scan (`:29`) in `getCorrectionEligibleStaff()`, and two money sites, the today-summary totals (`:936`) and its payment breakdown (`:947`).
