...
*   **Consumed by:** `backend/routes/transactions.js` — two money sites: the today-summary totals (`:936`) and its payment breakdown (`:947`).
...
*   **`backend/routes/transactions.js:21` and `:29`** — `getCorrectionEligibleStaff()`, which chooses the default replacement masseuse when reception corrects a transaction. Its workload subquery and busy scan filter live rows only, so a masseuse mid-massage on a correction replacement reads as free with zero workload and is offered as the replacement for the next correction. That is `00-project-docs/feature-specifications/transaction-correction-operational-reversal.md` FR-004 failing in the very flow this epic is about. **Owned by `ETSC-CORE-002b`**, not by `ETSC-CORE-002`, and sequenced immediately after it because both steps write that file.
*   **`backend/routes/transactions.js:791` and `:800`** — the audit-integrity repair tool. **Owned by `ETSC-CORE-002a`.**
