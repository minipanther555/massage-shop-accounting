...
### The two reports now agree on an edited day, and one recorded reason they still would not (ETSC-MONEY-001, 2026-08-18)
- **No production change.** `ETSC-CORE-002` had already converted `GET /summary/today` to `countsAsLiveWork()`. This step is the guard that the agreement survives, and the proof that the fee side of an edit is right.
- **What agreement is worth pinning.** Before the epic, `GET /summary/today` counted `ACTIVE` only while `GET /financial` had always counted `ACTIVE` and `CORRECTED` (`:239`), so every edited transaction was money the manager saw on one screen and not the other. `tests/integration/edited-transaction-money-agrees.integration.test.js` drives a real edit through `POST /api/transactions` and asserts both endpoints return the edited amount — **not merely the same amount**. Two equally wrong reports agree, and so does an empty day, so the expected figure is named, the superseded figure and the both-rows-counted figure are named as forbidden, and the chain is proved to exist first.
- **🔴 The one reason they can still disagree, deliberately unfixed.** `GET /summary/today` applies `isSettled()` (`:205`) and `GET /financial` does not (`:239` has no such term), so the two differ by the value of any **part-paid add-on** — for reasons that have nothing to do with editing. That is a pre-existing defect recorded in `00-project-docs/steps/edited-transaction-state-correctness-steps.md` Discoveries and out of this epic's scope. The fixture therefore carries no add-on and no unsettled row, and a guard asserts it: with every row settled the two filters cannot disagree, so any difference the agreement assertion finds is about editing.
- **The payday side.** `staff.total_fees_earned` is written by the edit path, not by these queries (`backend/routes/transactions.js:681-684` reverses the original, `:719-725` accrues the replacement), so the spec drives real edits rather than hand-building rows — a hand-built fixture would assert its own arithmetic. It pins that one edit leaves the edited fee (not the sum, not zero, not the difference), that two successive edits leave the last fee only, that correcting the **masseuse** moves the whole fee and leaves the wrongly-credited one at zero, and that the accrued balances reconcile against the fees on the day's live rows and against `base_fee_total`.
- **Verified by mutation, since the behaviour was already correct.** Narrowing `countsAsLiveWork()` back to `status = 'ACTIVE'` fails the four report assertions and none of the payday ones; removing the fee reversal fails the four payday assertions and none of the report ones. The two halves are independent, and the reconciliation assertion is the only one that catches both. Both mutations reverted.
