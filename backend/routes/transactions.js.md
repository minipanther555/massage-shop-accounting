...
*   **`POST /add-ons`**
    *   **Purpose:** Creates a paid add-on (extra time, or an extra service) linked to an original sale, charging only the additional amount owed. Paid Time Extension `PTE-API-001`. It also records the two kinds of money that are **not** a service — a tip and a miscellaneous charge — which take a separate path inside the same route (`RIT-MONEY-001`, below).
    *   **Parameters (Body):** `add_on_kind` is one of `DURATION_UPGRADE` | `ADDITIONAL_SERVICE` | `TIP` | `MISC_INCOME`; anything else is `400`. **For the two service kinds:** `parent_transaction_id` required; `service_type`, `location`, `duration` required; `payment_status` (`PAID` | `PENDING`, default `PAID`); `payment_method` required only when `PAID`; optional `masseuse_name` (honoured for `ADDITIONAL_SERVICE` only), `customer_contact`, `start_datetime`, `end_datetime`, `time_window_promotion_override`.
    ...
    *   **Logic — `TIP` and `MISC_INCOME` (`RIT-MONEY-001`, `createMoneyOnlyAddOn`):** These branch out **before** the service checks, so the two service kinds are untouched. They take `amount` (required, positive — there is no catalog price to derive it from) and `description` — **required and non-blank for `MISC_INCOME`, optional for `TIP`** (`RIT-MONEY-003`). `FR-006`'s Failure Modes require a miscellaneous charge to say what it was for, and a `MISC_INCOME` row's description IS its `service_type`, so an empty one would print the enum name `MISC_INCOME` in every ledger view. `FR-005` states no such rule for a tip, which is self-describing and already has an explicit `ทิป (tip)` fallback for its expense row. The description is trimmed, so whitespace does not satisfy the check. Both rejections are `400` and happen **before any write**; `service_type`, `location` and `duration` are ignored. A `TIP` must name a parent (`400` without one); a `MISC_INCOME` may or may not, per the feature spec's §6 Transaction Ledger constraint. A supplied parent must exist (`404`), must not itself be an add-on (`400`), and must be **live work** — `ACTIVE` **or** `CORRECTED`, tested with the shared `isLiveWork()` predicate rather than a status literal, so a tip is still accepted on a transaction that has since been edited (`FR-005` edge case) while the superseded `EDITED (Corrected by …)` original and every cancelled row are refused (`409`). **This is deliberately looser than the service path's `ACTIVE`-only rule, which is unchanged.** The row is written with `masseuse_fee` of `0` and **no** `staff.total_fees_earned` update — a tip never reaches a payday balance. `service_type` carries the description (falling back to the kind), `masseuse_name` and `location` are the empty string when nothing applies, `duration` is `0` and `start_time` = `end_time` = now, a zero-length window that can never make a masseuse read busy. `countsAsMassage()` already excludes both kinds, so neither moves a queue position or a workload count. **A `TIP` also writes the expense side, in the SAME database transaction** (`RIT-MONEY-002`): one `expenses` row of the same amount, carrying `masseuse_name` and `business_day`, with `date` set to the UTC calendar day exactly as `backend/routes/expenses.js` fills it for an ordinary expense — every existing expense reader, the end-day archive total and its `DELETE FROM expenses WHERE date = ?` all key on `date`, so a tip lands in the same day as every other expense. The expense `description` ends with the tip's own transaction id, which is the only link between the two halves — `expenses` has no foreign key to `transactions`. The pair is written under `BEGIN IMMEDIATE TRANSACTION` … `COMMIT`, with `ROLLBACK` in the catch, matching the correction workflow's own pattern in this file; `FR-005`'s Failure Modes: "Both rows are written in one database transaction; a failure writes neither." **`MISC_INCOME` writes NO expense row** (`FR-006`), and neither kind ever updates `staff.total_fees_earned`.
