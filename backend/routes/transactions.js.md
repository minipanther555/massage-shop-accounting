...
### `GET /summary/today` did not count an edited transaction (2026-08-18)
- **Bug Summary:** This router's own today-summary endpoint reported zero revenue and zero fees for any customer whose transaction had been edited, the same defect that hit every money figure in `backend/routes/reports.js`.
- **Validated Hypothesis:** Both of its queries — the totals at `:936` and the payment breakdown at `:947` — filtered `status = 'ACTIVE'`, and after an edit neither row of the chain is `ACTIVE`: the original is relabelled `EDITED (Corrected by <id>)` at `:693-696` and the replacement is inserted `CORRECTED` at `:713`.
- **Invalidated Hypotheses:** That this endpoint was dead code and could be skipped. It has no `web-app` caller today — `web-app/api.js:184` uses `/reports/summary/today` instead — but it is a live, authenticated, documented endpoint (`backend/README.md:112`), and leaving one of two identically-named summaries wrong is exactly how the defect survives a partial fix. It was fixed on the same evidence as the rest.
- **Resolution:** Both queries now call `countsAsLiveWork()` from `backend/services/transaction-status-sql.js`, giving `status IN ('ACTIVE', 'CORRECTED')` — the same allowlist the date-range reports and the unique booking index already used.

**Deliberately not changed in this router.** The edit write path at `:693-696` and `:713` keeps its status values, which are mandated by `00-project-docs/feature-specifications/transaction-correction-operational-reversal.md` FR-003 and §6 — the defect was in the readers, not the writer. The audit-integrity repair tool at `:791` and `:800` is owned by `ETSC-CORE-002a`. The add-on and cancellation eligibility guards (`:291`, `:437`, `:452`, `:494`, `:504`, `:878`, `:893`) test a row's own lifecycle state rather than aggregating money. `getCorrectionEligibleStaff()` carried the same live-row defect and **was fixed by `ETSC-CORE-002b`** — see the next section.

### The correction flow offered a masseuse who was mid-massage (2026-08-18)
- **Bug Summary:** `getCorrectionEligibleStaff()` (`:16-45`) decides who replaces a masseuse when reception corrects a transaction. A masseuse who was mid-massage **on a correction replacement** read as free with zero workload, so she was ranked first as the default replacement and — worse — an override naming her was **accepted** instead of refused. That is `00-project-docs/feature-specifications/transaction-correction-operational-reversal.md` FR-004 and FR-007 failing inside the correction flow itself.
- **Validated Hypothesis:** Both of the function's filters read live rows only — the workload subquery at `:21` and the busy scan at `:29`. After an edit neither row of the chain is `ACTIVE`, so the replacement massage was invisible to both. Same defect class as `ETSC-CORE-001` fixed in `backend/routes/staff.js`, in a different file.
- **Invalidated Hypotheses:** That fixing the busy scan alone was enough. It is not: it removes the mid-massage masseuse from the list, which hides the workload defect rather than fixing it. A masseuse whose correction replacement had already **finished** still scored zero and was handed the next correction ahead of colleagues who had done less work.
- **Resolution:** Both filters now call `countsAsLiveWork()` from `backend/services/transaction-status-sql.js`, giving `status IN ('ACTIVE', 'CORRECTED')`. No second predicate, no inline SQL, no new import — `:8` already carried it from `ETSC-CORE-002`. Query plans are byte-identical before and after: the workload subquery keeps its seek on `idx_transactions_business_day_staff (business_day, masseuse_name, status)`, which SQLite serves as an index seek for `IN` just as it did for `=`.

### The audit-repair tool matched on status and missed three kinds of superseded row (2026-08-18)
- **Bug Summary:** `POST /api/transactions/fix-edited-status` is a fraud control — it finds rows that an edit superseded but that were left counting as live, and relabels them. It keyed on **status** in two places and both were too narrow, so the tool silently repaired nothing in three distinct situations.
- **Validated Hypothesis:** The outer scan took `corrected_from_id IS NOT NULL` plus a live status, and the inner lookup required the superseding row to read exactly `CORRECTED`. That misses (1) the original of a **first** edit, whose `corrected_from_id` is null by definition — the most basic case the tool exists for; (2) a superseded row left as a correction replacement rather than relabelled; (3) any chain edited twice, where the superseding row has itself since been relabelled.
- **🔴 Why (2) stopped being harmless:** before `ETSC-CORE-001` and `ETSC-CORE-002`, a stray superseded row marked `CORRECTED` counted for nothing anywhere and was inert. `countsAsLiveWork()` now admits `CORRECTED` at fourteen sites, so that same row adds a **second massage to the masseuse's workload and a second fare to the day's takings**. This epic converted a dormant gap into a live double-count, in the one tool meant to catch double-counting. Reproduced: the roster reported `today_massages: 2` for one massage before repair.
- **Invalidated Hypotheses:** That only the inner lookup needed changing, which is what the step's original Validation line reached. Fixing it alone leaves (1) and (2) — both real, and (2) newly dangerous.
- **Resolution:** One join replaces both status matches. A row is superseded **iff another row points at it through `corrected_from_id`**, so the query joins successors to predecessors on that link and filters the predecessor with `countsAsLiveWork('t')` — an allowlist choosing *which* rows may be relabelled, which is what keeps a `CANCELLED (…)` row's own reason string from being overwritten. `ORDER BY s.id ASC` into a `Map` makes the choice deterministic if a chain ever forked. Query plan improved: the old scan was `SCAN transactions`; the new one is `SCAN s | SEARCH t USING INDEX sqlite_autoindex_transactions_1 (transaction_id=?)`.

**🔴 The default-suggestion branch is unreachable over HTTP.** `getCorrectionEligibleStaff()` drives two things at `:650-655`: `eligibleStaff[0]` is the default replacement when the request carries no `masseuse_name`, and `.some(...)` is the FR-007 availability guard. **Only the guard can run.** `validateInput` is mounted globally at `backend/server.js:86` and `backend/middleware/input-validation.js:105-108` rejects any `POST /api/transactions` whose body has no `masseuse_name`, correction or not — measured, returning `400 "Masseuse name must be between 1 and 100 characters"`. So `:652` never executes, and the sort at `:45` has no HTTP-visible effect. The route's own field check at `:588` deliberately exempts corrections (`!masseuseName && !originalTransactionId`), so the route and the middleware disagree about whether a correction may omit the masseuse. The workload filter was fixed anyway — it is wrong either way and is a live trap for whoever makes the default reachable — and is asserted directly against the function rather than through the endpoint. `router.getCorrectionEligibleStaff` is exported for that assertion only; it changes no runtime behaviour and leaves `module.exports = router` intact. **FR-004's "the default is the next eligible staff member" is therefore not exercised by any reachable path today.** Recorded as a Discovery in `00-project-docs/steps/edited-transaction-state-correctness-steps.md` for the operator to schedule or decline; not fixed here, because relaxing an input-validation contract is outside this step's Touches.

### The edit is atomic, and that is now pinned by a test (ETSC-CORE-003, 2026-08-18)
- **No production change.** `POST /` already wraps the whole edit in one database transaction — `BEGIN IMMEDIATE` at `:659`, `COMMIT` at `:755`, `ROLLBACK` at `:772` — with the original masseuse's fee reversal (`:681-684`), the relabel of the superseded row (`:692-695`) and the insert of the replacement (`:714`) all inside that span. The step confirmed this by reading the handler and found no escape path, so it shipped as tests only.
- **Why it needed covering anyway.** Those three writes are one unit of work. A partial write leaves an original relabelled `EDITED (Corrected by <id>)` pointing at a replacement row that does not exist — a chain with **no** live row, so the masseuse loses the massage from her workload and the shop loses the money from the day. Nothing in the repo asserted that the wrapper stays.
- **How the coverage forces a failure.** `tests/integration/edit-is-all-or-nothing.integration.test.js` creates a SQLite trigger that aborts the INSERT at STEP 7 on a marker `customer_contact`, then drops it. `RAISE(ABORT)` reverts only the offending statement and leaves the enclosing transaction open, so the test observes this handler's own `ROLLBACK` undoing the fee reversal and the relabel. The load-bearing assertion is that the original masseuse's `total_fees_earned` is restored: "the original is still `ACTIVE` and there is no replacement" would also be true of a handler that never ran, or one that inserted before relabelling. A source-order assertion pins that second case from the other side.
- **Verified by mutation, since the behaviour was already correct.** Removing `BEGIN IMMEDIATE` and `COMMIT` makes the atomicity assertions fail with the exact partial-write state described above — the original left `EDITED (Corrected by TX-…)` with no such row in the table. Reverted immediately; the mutation is not in the repository.
