...
function loadTransactionForCorrection() {
  ...
  function setupReportsSheet(ss) {
    ...
    // Header
    sheet.getRange("A1").setValue("MANAGEMENT DASHBOARD").setFontWeight("bold").setFontSize(16);
    ...
    // SECTION 2: THIS WEEK'S MASSEUSE FEES (for Sunday payments)
    sheet.getRange("A10").setValue("THIS WEEK'S MASSEUSE FEES (Sunday Payment)").setFontWeight("bold").setFontSize(14);
    ...
    sheet.getRange("A11").setFormula(weeklyFeesFormula);
    
    // SECTION 3: LAST WEEK'S MASSEUSE FEES
    sheet.getRange("A17").setValue("LAST WEEK'S MASSEUSE FEES").setFontWeight("bold").setFontSize(14);
    const lastWeekStart = `=TODAY()-WEEKDAY(TODAY(),2)-6`; // Monday of last week
    const lastWeekEnd = `=TODAY()-WEEKDAY(TODAY(),2)`;     // Sunday of last week
    const lastWeekFeesFormula = `=IFERROR(QUERY('${CONFIG.sheets.masterLog}'!A:L, "SELECT D, SUM(H), COUNT(H) WHERE (L CONTAINS 'ACTIVE' OR L CONTAINS 'CORRECTED') AND C >= DATE '"&TEXT(${lastWeekStart},"yyyy-mm-dd")&"' AND C <= DATE '"&TEXT(${lastWeekEnd},"yyyy-mm-dd")&"' GROUP BY D ORDER BY SUM(H) DESC LABEL D 'Masseuse', SUM(H) 'Last Week Fees', COUNT(H) 'Last Week Massages'", 1), "No data last week.")`;
    sheet.getRange("A18").setFormula(lastWeekFeesFormula);
    
    // SECTION 4: THIS MONTH'S BREAKDOWN
    sheet.getRange("A24").setValue("THIS MONTH'S BREAKDOWN").setFontWeight("bold").setFontSize(14);
    ...
    sheet.getRange("A25").setFormula(monthlyRevenueFormula);
    
    // SECTION 5: DAILY BREAKDOWN (Last 7 Days)
    sheet.getRange("A31").setValue("LAST 7 DAYS BREAKDOWN").setFontWeight("bold").setFontSize(14);
    ...
    sheet.getRange("A32").setFormula(dailyBreakdownFormula);
    
    // SECTION 6: SERVICE TYPE PERFORMANCE
    sheet.getRange("A38").setValue("SERVICE TYPE PERFORMANCE").setFontWeight("bold").setFontSize(14);
    ...
    sheet.getRange("A39").setFormula(servicePerformanceFormula);
