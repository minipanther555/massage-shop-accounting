// Usage:
//   BASE=https://109.123.238.197.sslip.io node scripts/prod-capture-429-auth.js
// Optional envs:
...
const OUTDIR = process.env.OUT || `diagnostics/prod-429-auth__${new Date().toISOString().replace(/[:.]/g,'-')}`;
...
(async () => {
  ...
  const loginRes = await page.request.post(`${BASE}/api/auth/login`, {
    ...
    data: JSON.stringify({ username: 'manager', password: 'manager456' })
  });
  
  log({ kind: 'login_attempt', status: loginRes.status() });
  ...
  const allstaffRes = await page.request.get(`${BASE}/api/staff/allstaff`, { ignoreHTTPSErrors: true }).catch(()=>null);
  let allNames = [];
