...
function setupDailyEntrySheet(ss) {
  ...
  for (let i = 3; i <= CONFIG.rosterSize + 2; i++) {
    ...
    sheet.getRange(`D${i}`).setFormula(countFormula);
  }
  ...
  // SECTION 5: RECENT TRANSACTIONS LOG (A27:I31) - CLEARLY SEPARATED
  applyHeaderStyle(sheet.getRange("A27:I27").merge().setValue("RECENT TRANSACTIONS LOG"));
  
  // Fixed QUERY formula - shows Payment Method instead of Transaction ID, limited to 4 rows
  ...
  const recentLogFormula = `=IFERROR(QUERY('${CONFIG.sheets.masterLog}'!A:L, "SELECT G, D, E, F, H, I, J, K, L WHERE L CONTAINS 'ACTIVE' ORDER BY B DESC LIMIT 4 FORMAT F '฿#,##0.00', H '฿#,##0.00', I 'h:mm am/pm', J 'h:mm am/pm' LABEL G 'Payment Method', D 'Masseuse', E 'Service', F 'Amount', H 'Fee', I 'Start Time', J 'End Time', K 'Contact'", 1), "No transactions yet.")`;
