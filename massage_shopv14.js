...
const CONFIG = {
  ...
  rosterSize: 20,
  buttons: {
    submit: 'G13', load: 'G20', next: 'E1', endDay: 'G25'
  }
};

const THEME = {
  headerBackground: '#4a86e8',
  headerFontColor: '#ffffff',
  headerFontSize: 12,
  headerFontWeight: 'bold',
  subHeaderBackground: '#f3f3f3',
  buttonBackground: '#34a853',
  correctionModeBackground: '#fff2cc'
};
...
function onEdit(e) {
  ...
  try {
    ...
    if (value === 'TRUE') {
      ...
      } else if (cell.startsWith('J')) {
        console.log('12. Expense button clicked');
        try {
          console.log('13. About to call addExpense');
          addExpense(cell);
          console.log('14. ✅ addExpense completed');
        } catch (expenseError) {
          console.log('14. ❌ Add expense error:', expenseError);
        }
      } else {
...
function endDay() {
  ...
  if (response === ui.Button.YES) {
    try {
      // Collect today's expenses first
      const dailyExpenses = collectDailyExpenses();

      // Archive today's data to Daily Summary (including expenses)
      archiveDailyData(dailyExpenses);
...
function archiveDailyData(dailyExpenses) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  // Add to Daily Summary (including expenses)
...
function resetDailyEntrySheet() {
  ...
  // Clear expense fields (H3:K15)
  dailyEntrySheet.getRange('H3:K15').clearContent();

  // Restore the recent transactions log header styling
  dailyEntrySheet.getRange('A27:I27').setBackground(THEME.headerBackground).setFontColor(THEME.headerFontColor).setFontSize(THEME.headerFontSize)
    .setFontWeight(THEME.headerFontWeight)
    .setHorizontalAlignment('center');

  // Restore today's quick summary header styling
  dailyEntrySheet.getRange('A32:I32').setBackground(THEME.headerBackground).setFontColor(THEME.headerFontColor).setFontSize(THEME.headerFontSize)
    .setFontWeight(THEME.headerFontWeight)
    .setHorizontalAlignment('center');
...
function submitTransaction() {
  ...
  const [masseuse, service, paymentMethod, startTime, endTime] = [entryData[0][0], entryData[1][0], entryData[2][0], entryData[3][0], entryData[4][0]];
  const customerContact = String(entryData[5][0] || '');

  // Debug: Log the values to see what's causing issues
  console.log('Submit values:', {
    masseuse, service, paymentMethod, startTime, endTime, customerContact
  });
...
function addExpense(cellAddress) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const entrySheet = ss.getSheetByName(CONFIG.sheets.dailyEntry);
  const ui = SpreadsheetApp.getUi();

  // Get the row number from the cell address (e.g., J5 -> row 5)
  const row = parseInt(cellAddress.substring(1));

  // Get the expense description and amount from the same row
  const description = entrySheet.getRange(`H${row}`).getValue();
  const amount = entrySheet.getRange(`I${row}`).getValue();

  // Validate the expense data
  if (!description || !amount || amount <= 0) {
    ui.alert('Error', 'Please enter both a description and a valid amount for the expense.');
    return;
  }

  // Mark the expense as added
  entrySheet.getRange(`K${row}`).setValue('Added');

  // Clear the description and amount fields for the next expense
  entrySheet.getRange(`H${row}`).clearContent();
  entrySheet.getRange(`I${row}`).clearContent();

  // Reset the checkbox
  entrySheet.getRange(cellAddress).setValue(false);

  ui.toast(`Expense added: ${description} - ฿${amount}`);
}

function collectDailyExpenses() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const entrySheet = ss.getSheetByName(CONFIG.sheets.dailyEntry);
  ...
  // Check all expense rows (H3:H15) for added expenses
  for (let row = 3; row <= 15; row++) {
    const status = entrySheet.getRange(`K${row}`).getValue();

    if (status === 'Added') {
      const description = entrySheet.getRange(`H${row}`).getValue() || 'Unknown expense';
      const amount = entrySheet.getRange(`I${row}`).getValue() || 0;
...
function applyHeaderStyle(range) {
  ...
  range.setBackground(THEME.headerBackground).setFontColor(THEME.headerFontColor).setFontSize(THEME.headerFontSize).setFontWeight(THEME.headerFontWeight)
    .setHorizontalAlignment('center');
}
...
function setupDailySummarySheet(ss) {
  ...
  // Headers
  const headers = ['Date', 'Total Revenue', 'Total Fees', 'Total Transactions', 'Total Expenses'];
  applyHeaderStyle(sheet.getRange('A1:E1').setValues([headers]));
...
function setupDailyEntrySheet(ss) {
  ...
  sheet.getRange('D1').setValue('Serve Next Customer ->').setFontWeight('bold').setHorizontalAlignment('right')
    .setVerticalAlignment('middle');
  sheet.getRange(CONFIG.buttons.next).insertCheckboxes().setVerticalAlignment('middle');

  // Roster headers and data
  const rosterHeaders = [['Position', 'Masseuse Name', 'Status', "Today's Massages"]];
  ...
  sheet.getRange('E13:F13').merge().setValue('Click to Submit ->').setBackground(THEME.buttonBackground)
  ...
  sheet.getRange('E20:F20').merge().setValue('Click to Load ->').setBackground('#f4b400')
    .setFontColor('black')
    .setFontWeight('bold')
    .setHorizontalAlignment('right')
    .setVerticalAlignment('middle');
  ...
  sheet.getRange('E25:F25').merge().setValue('Click to End Day ->').setBackground('#e06666')
    .setFontColor('white')
    .setFontWeight('bold')
    .setHorizontalAlignment('right')
    .setVerticalAlignment('middle');
  ...
  // SECTION 7: DAILY EXPENSES (H1:K15) - NEW ADDITION
  applyHeaderStyle(sheet.getRange('H1:K1').merge().setValue('DAILY EXPENSES'));

  // Expense headers
  sheet.getRange('H2').setValue('Description:').setFontWeight('bold');
  sheet.getRange('I2').setValue('Amount:').setFontWeight('bold');
  sheet.getRange('J2').setValue('Add:').setFontWeight('bold');
  sheet.getRange('K2').setValue('Status:').setFontWeight('bold');

  // Expense input fields (rows 3-15)
  sheet.getRange('H3:H15').setBackground(THEME.subHeaderBackground);
  sheet.getRange('I3:I15').setBackground(THEME.subHeaderBackground).setNumberFormat('"฿"#,##0.00');

  // Add expense buttons (checkboxes)
  sheet.getRange('J3:J15').insertCheckboxes().setHorizontalAlignment('center');

  // Status column for tracking
  sheet.getRange('K3:K15').setFontStyle('italic').setFontColor('#666666');
  ...
  sheet.protect().setUnprotectedRanges([
    ...
    sheet.getRange('F18'),
    sheet.getRange('H3:I15'),
    sheet.getRange('J3:J15')
