...
describe('Transaction Rendering Component', () => {
  ...
  test('should apply edited-transaction class to transactions with EDITED status', () => {
    ...
    const editedTransaction = transactionItems[1]; // The EDITED transaction
    const activeTransaction = transactionItems[2]; // The ACTIVE transaction
    const correctedTransaction = transactionItems[3]; // The CORRECTED transaction
    ...
    // The CORRECTED transaction should NOT have the edited-transaction class
    expect(correctedTransaction.classList.contains('edited-transaction')).toBe(false);
  });
