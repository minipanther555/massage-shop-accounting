...
async function runComprehensiveFunctionalTest() {
  ...
  try {
    ...
    page.on('requestfailed', (request) => {
      ...
      failedRequests.push({
        url, status: 'FAILED', contentType: 'N/A', reason: failure?.errorText || 'Unknown failure'
      });
    ...
    const loginSuccess = await page.evaluate(() => window.location.href.includes('index.html'));
