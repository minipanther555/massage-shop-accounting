...
test('remove button works without crashes and repopulates dropdown (AGFSM)', async ({ page }) => {
  const consoleErrors = [];
  const pageErrors = [];
  const requests = [];
  ...
  const criticalErrors = [...consoleErrors, ...pageErrors].filter(err => 
    ...
    err.includes('Cannot redefine property') ||
  ...
  // Test PWTEST hook if available
  const testHook = await page.evaluate(() => {
    return typeof window.__staffTest !== 'undefined' ? window.__staffTest.getState() : null;
  });
  
  if (testHook) {
    console.log('PWTEST hook working:', testHook);
    expect(testHook.ALL_STAFF).toBeDefined();
    expect(testHook.CURRENT_ROSTER).toBeDefined();
  }
});
