...
function endDay() {
  ...
  if (response === ui.Button.YES) {
    try {
      ...
      ui.toast('Day ended successfully. Data archived and sheet reset for tomorrow.', 'Success', 5);
    } catch (error) {
...
function archiveDailyData() {
  ...
  dailySummarySheet.appendRow([
    ...
    dailyData.revenueByType,
    dailyData.feesByMasseuse,
    dailyData.massageCountByMasseuse
  ]);
}
...
function calculateDailyTotals(dateStr) {
  ...
  const masseuseCol = headers.indexOf("Masseuse Name");
  const serviceCol = headers.indexOf("Service Type");
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    if (rowDate === dateStr && row[statusCol] === "ACTIVE") {
      ...
      const masseuse = row[masseuseCol] || "";
      ...
      if (service) {
        revenueByType[service] = (revenueByType[service] || 0) + revenue;
      }
      
      // Track by masseuse
      if (masseuse) {
        feesByMasseuse[masseuse] = (feesByMasseuse[masseuse] || 0) + fees;
        massageCountByMasseuse[masseuse] = (massageCountByMasseuse[masseuse] || 0) + 1;
      }
    }
  }
  ...
  return {
    ...
    totalFees,
    totalTransactions,
    revenueByType: JSON.stringify(revenueByType),
    feesByMasseuse: JSON.stringify(feesByMasseuse),
    massageCountByMasseuse: JSON.stringify(massageCountByMasseuse)
  };
}
...
function resetDailyEntrySheet() {
  ...
  // Clear recent transactions log
  dailyEntrySheet.getRange("A26:I30").clearContent();
}
...
function loadTransactionForCorrection() {
  ...
  function setupDailySummarySheet(ss) {
    ...
    // Headers
    const headers = ["Date", "Total Revenue", "Total Fees", "Total Transactions", "Revenue by Type", "Fees by Masseuse", "Massage Count by Masseuse"];
    applyHeaderStyle(sheet.getRange("A1:G1").setValues([headers]));
...
function setupDailyEntrySheet(ss) {
  ...
  let rosterHeaders = [["Position", "Masseuse Name", "Status"]];
  for (let i = 1; i <= CONFIG.rosterSize; i++) { 
    rosterHeaders.push([`#${i}`, "", "Available"]); 
  }
  sheet.getRange(2, 1, CONFIG.rosterSize + 1, 3).setValues(rosterHeaders);
  ...
  const recentLogFormula = `=IFERROR(QUERY('${CONFIG.sheets.masterLog}'!A:L, "SELECT A, C, D, E, F, H, I, J, K, L WHERE L CONTAINS 'ACTIVE' ORDER BY B DESC LIMIT 5 FORMAT C 'yyyy-mm-dd', F '฿#,##0.00', H '฿#,##0.00', I 'h:mm am/pm', J 'h:mm am/pm'", 1), "No transactions yet.")`;
